...
import { detectSheet, warpToCanonical } from './opencv/documentDetection'
...
const STAGE_WEIGHTS: { stage: ScanStage; label: string; weight: number }[] = [
  ...
  { stage: 'detecting-sheet', label: 'Detectando hoja', weight: 15 },
...
async function loadImageToMat(cv: any, file: File): Promise<any> {
...
export async function scanPage(
  ...
  const sheet = detectSheet(cv, srcMat)

  emit('correcting-image')
  const canonicalMat = warpToCanonical(cv, srcMat, sheet.corners, calibration.canonicalWidth, calibration.canonicalHeight)
  const { displayMat, ocrMat } = enhanceForReading(cv, canonicalMat)
